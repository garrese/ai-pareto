...
coding agents. Entries use the local project date (`YYYY-MM-DD`), with the newest date first. Every
entry is a change, so the file intentionally has no change-type subsections. Merge commits are not
listed separately because the evolution they integrate appears once under its work commit.
