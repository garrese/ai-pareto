...
const environment = {
  ...
  COLLECTOR_DIAGNOSTICS_BUCKET: 'diagnostics-bucket',
  AA_API_KEY: 'test-key',
...
test('cloud collector config applies safe production defaults', () => {
  ...
  assert.equal(config.diagnosticsBucketName, 'diagnostics-bucket');
...
test('cloud collector config requires one task and all external resource names', () => {
  ...
  assert.throws(
    () => loadCloudCollectorConfig({ ...environment, COLLECTOR_DIAGNOSTICS_BUCKET: '' }),
    /COLLECTOR_DIAGNOSTICS_BUCKET/,
  );
});
